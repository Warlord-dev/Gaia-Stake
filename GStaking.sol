...
contract GaiaLPStaking is ERC20, Ownable, ReentrancyGuard {
    ...
    function fund(address _from, uint256 _amount) public {
        ...
        require(gaiaTokenContract.balanceOf(_from) >= _amount, 'fund: expected an address that contains enough Gaia for Transfer');
        gaiaTokenContract.transferFrom(_from, address(this), _amount);
    ...
    function safeGaiaTransfer(address _to, uint256 _amount) internal {
        ...
        uint256 gaiaBal = gaiaTokenContract.balanceOf(_from);
        if (_amount > gaiaBal) {
            gaiaTokenContract.transfer(_to, gaiaBal);
        } else {
            gaiaTokenContract.transfer(_to, _amount);
        }
    }
}